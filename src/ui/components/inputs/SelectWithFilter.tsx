...
export const SelectWithFilter = ({ items, select, selectionArr }: SelectWithFilterProps) => {
  ...
  return (
    <div ref={selectRef} className="min-w-52 max-w-full relative">
      ...
      <ul className={`bg-slate-100 py-2 px-2  absolute w-full z-10 max-h-36 overflow-auto scale-0 ${isOpen ? 'scale-100' : 'scale-0'} transition-all duration-150`}>
        {
          (items.length == 0)
            ? 'Esta vacío'
            : items
              ...
              .map((item) => (
                <li onClick={() => select(item.id)} key={item.id.toString()} className="hover:bg-slate-200 px-4 py-2 cursor-pointer">
                  <div className="flex items-ceter gap-4">
                    <Checkbox onChange={() => {}} checked={selectionArr.indexOf(item.id) !== -1 ? true : false} />
                    {item.title}
                  </div>
                </li>
              ))
